 ...
 * @brief  TF发布函数
 */
void sensor_ros::publishTF(void)
{
    cartographer::transform::Rigid3d T_map_odom=  m_localization.robot_pose.inverse();
    tf::Transform tf_base2map_;
    tf_base2map_ =
            tf::Transform(
...
void sensor_ros::laser_onewave_callback(sensor_msgs::LaserScanConstPtr msg)
{
    // 构造点云数据
    cartographer::sensor::PointCloudWithIntensities point_cloud;
    // 计算当前时间,公式和FromRos函数一致
    ...
    for (size_t i = 0; i < msg->ranges.size(); ++i)
    {
        // 获取一个激光数据
        const auto& echoes = msg->ranges[i];
        const float first_echo = echoes;// 当前激光的距离
        if (msg->range_min <= first_echo && first_echo <= msg->range_max)
        {// 确保激光数据在范围内
            // 绕z轴旋转角度 angle
            const Eigen::AngleAxisf rotation(angle, Eigen::Vector3f::UnitZ());
            ...
            {
                // 压入光强数据
                auto echo_intensities = msg->intensities[i]*100;
                point_cloud.intensities.push_back(echo_intensities);
            }
            else
            {
                point_cloud.intensities.push_back(100.);
            }
        }
        // 更新当前激光角度
        angle += msg->angle_increment;
...
/*
 * @brief  将ROS格式odom 数据转换到 cartographer 格式的数据
 * @param msg odom 数据
 * @return cartographer格式数据
 */
std::unique_ptr<cartographer::sensor::OdometryData> sensor_ros::ToOdomData(
 ...
 * @param msg 轮速计数据
 */
void sensor_ros::odometry_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
    std::unique_ptr<cartographer::sensor::OdometryData> odom_data = ToOdomData(msg);
    if(odom_data != nullptr)
    {
        m_localization.HandleOdometryMessage("odom",odom_data->time,odom_data->pose);
    }
}
